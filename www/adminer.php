...
edit_form($a,$p,$J,$zi){global$b,$x,$ci,$n;$Dh=$b->tableName(table_status1($a,true));page_header(($zi?'Upravit':'VloÅ¾it'),$n,array("select"=>array($a,$Dh)),$Dh);if($J===false)echo"<p class='error'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
lang($hi,$Ye=null){if(is_array($hi)){$Uf=($Ye==1?0:($Ye&&$Ye<5?1:2));$hi=$hi[$Uf];}$hi=str_replace("%d","%s",$hi);$Ye=format_number($Ye);return
...
Min_DB;$i=$b->credentials();if($f->connect($i[0],$i[1],$i[2])){if(min_version(9,0,$f)){$f->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$f)){$wh['Å˜etÄ›zce'][]="json";$ri["json"]=4294967295;if(min_version(9.4,0,$f)){$wh['Å˜etÄ›zce'][]="jsonb";$ri["jsonb"]=4294967295;}}}return$f;}return$f->error;}function
get_databases(){return
...
set_schema($Qg){global$f,$ri,$wh;$I=$f->query("SET search_path TO ".idf_escape($Qg));foreach(types()as$U){if(!isset($ri[$U])){$ri[$U]=0;$wh['UÅ¾ivatelskÃ© typy'][]=$U;}}return$I;}function
...
unconvert_field($o,$I){return$I;}function
support($Oc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Oc);}$x="oracle";$ri=array();$wh=array();foreach(array('ÄŒÃ­sla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a Äas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Å˜etÄ›zce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¡rnÃ­'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$ri+=$X;$wh[$y]=array_keys($X);}$yi=array();$mf=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$id=array("length","lower","round","upper");$od=array("avg","count","count distinct","max","min","sum");$lc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$dc["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Xf=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
last_id(){global$f;return$f->last_id;}$x="elastic";$mf=array("=","query");$id=array();$od=array();$lc=array(array("json"));$ri=array();$wh=array();foreach(array('ÄŒÃ­sla'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'Datum a Äas'=>array("date"=>10),'Å˜etÄ›zce'=>array("string"=>65535,"text"=>65535),'BinÃ¡rnÃ­'=>array("binary"=>255),)as$y=>$X){$ri+=$X;$wh[$y]=array_keys($X);}}$dc=array("server"=>"MySQL")+$dc;if(!defined("DRIVER")){$Xf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
selectLinks($Ch,$O=""){global$x,$m;echo'<p class="links">';$qe=array("select"=>'Vypsat data');if(support("table")||support("indexes"))$qe["table"]='Zobrazit strukturu';if(support("table")){if(is_view($Ch))$qe["view"]='PozmÄ›nit pohled';else$qe["create"]='PozmÄ›nit tabulku';}if($O!==null)$qe["edit"]='NovÃ¡ poloÅ¾ka';$C=$Ch["Name"];foreach($qe
...
selectQuery($G,$sh,$Mc=false){global$x,$m;$I="</p>\n";if(!$Mc&&($Ri=$m->warnings())){$t="warnings";$I=", <a href='#$t'>".'VarovÃ¡nÃ­'."</a>".script("qsl('a').onclick = partial(toggle, '$t');","")."$I<div id='$t' class='hidden'>\n$Ri</div>\n";}return"<p><code class='jush-$x'>".h(str_replace("\n"," ",$G))."</code> <span class='time'>(".format_time($sh).")</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($G)."'>".'Upravit'."</a>":"").$I;}function
...
messageQuery($G,$Sh,$Mc=false){global$x,$m;restart_session();$ud=&get_session("queries");if(!$ud[$_GET["db"]])$ud[$_GET["db"]]=array();if(strlen($G)>1e6)$G=preg_replace('~[\x80-\xFF]+$~','',substr($G,0,1e6))."\n...";$ud[$_GET["db"]][]=array($G,time(),$Sh);$ph="sql-".count($ud[$_GET["db"]]);$I="<a href='#$ph' class='toggle'>".'SQL pÅ™Ã­kaz'."</a>\n";if(!$Mc&&($Ri=$m->warnings())){$t="warnings-".count($ud[$_GET["db"]]);$I="<a href='#$t' class='toggle'>".'VarovÃ¡nÃ­'."</a>, $I<div id='$t' class='hidden'>\n$Ri</div>\n";}return" <span class='time'>".@date("H:i:s")."</span>"." $I<div id='$ph' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($G,1000)."</code></pre>".($Sh?" <span class='time'>($Sh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ud[$_GET["db"]])-1)).'">'.'Upravit'.'</a>':'').'</div>';}function
...
edit_type($y,$o,$pb,$cd=array(),$Kc=array()){global$wh,$ri,$yi,$hf;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" aria-labelledby="label-type">';if($U&&!isset($ri[$U])&&!isset($cd[$U])&&!in_array($U,$Kc))$Kc[]=$U;if($cd)$wh['CizÃ­ klÃ­Äe']=$cd;echo
optionlist(array_merge($Kc,$wh),$U),'</select>
',on_help("getTarget(event).value",1),script("mixin(qsl('select'), {onfocus: function () { lastType = selectValue(this); }, onchange: editingTypeChange});",""),'<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' aria-labelledby="label-length">',script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});",""),'<td class="options">';echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'porovnÃ¡vÃ¡nÃ­'.')'.optionlist($pb,$o["collation"]).'</select>',($yi?"<select name='".h($y)."[unsigned]'".(!$U||preg_match(number_type(),$U)?"":" class='hidden'").'><option>'.optionlist($yi,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'PÅ™i zmÄ›nÄ›'.")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($cd?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'PÅ™i smazÃ¡nÃ­'.")".optionlist(explode("|",$hf),$o["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td id='label-comment'".($vb?"":" class='hidden'").">".'KomentÃ¡Å™':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.6.1")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>".script("row_count = ".count($p).";"),'</thead>
<tbody>
...
';edit_type("fields[$s]",$o,$pb,$cd);if($U=="TABLE"){echo'<td>',checkbox("fields[$s][null]",1,$o["null"],"","","block","label-null"),'<td><label class="block"><input type="radio" name="auto_increment_col" value="',$s,'"';if($o["auto_increment"]){echo' checked';}echo' aria-labelledby="label-ai">',script("qsl('input').onclick = function () { var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.oninput(); } }"),'</label><td>',checkbox("fields[$s][has_default]",1,$o["has_default"],"","","","label-default"),'<input name="fields[',$s,'][default]" value="',h($o["default"]),'" aria-labelledby="label-default">',script("qsl('input').oninput = function () { this.previousSibling.checked = true; }",""),(support("comment")?"<td".($vb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".(min_version(5.5)?1024:255)."' aria-labelledby='label-comment'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.6.1")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>&nbsp;".script("qsl('input').onclick = partial(editingAddRow, 1);","")."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=up.gif&version=4.6.1")."' alt='^' title='".'PÅ™esunout nahoru'."'>&nbsp;".script("qsl('input').onclick = partial(editingMoveRow, 1);","")."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=down.gif&version=4.6.1")."' alt='v' title='".'PÅ™esunout dolÅ¯'."'>&nbsp;".script("qsl('input').onclick = partial(editingMoveRow, 0);",""):""),($vf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.6.1")."' alt='x' title='".'Odebrat'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'fields\$1[field]');"):"");}}function
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".'ZmÄ›nit'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'PÅ™idat trigger'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('SchÃ©ma databÃ¡ze',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Fh=array();$Gh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$xe,PREG_SET_ORDER);foreach($xe
...
';if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'>".confirm(sprintf('Odstranit %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.6.1")."' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$ci,'">
</form>
...
';}if($C!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$C));}echo'<input type="hidden" name="token" value="',$ci,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$yf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$yf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($J["name"]);$Ga=" AS\n$J[select]";$A=ME."table=".urlencode($C);$Ee='Pohled byl zmÄ›nÄ›n.';$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$yf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$Ee);else{$Nh=$C."_adminer_".uniqid();drop_create("DROP $yf ".table($a),"CREATE $U ".table($C).$Ga,"DROP $U ".table($C),"CREATE $U ".table($Nh).$Ga,"DROP $U ".table($Nh),($_POST["drop"]?substr(ME,0,-1):$A),'Pohled byl odstranÄ›n.',$Ee,'Pohled byl vytvoÅ™en.',$a,$C);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($yf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?'PozmÄ›nit pohled':'VytvoÅ™it pohled'),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>KaÅ¾dÃ½ch<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Od,$J["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$uh,$J["STATUS"]),'<tr><th>KomentÃ¡Å™<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'Po dokonÄenÃ­ zachovat'),'</table>
<p>';textarea("EVENT_DEFINITION",$J["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="UloÅ¾it">
';if($aa!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$aa));}echo'<input type="hidden" name="token" value="',$ci,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?$_GET["name"]:$_GET["procedure"]);$Kg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$n){$vf=routine($_GET["procedure"],$Kg);$Nh="$J[name]_adminer_".uniqid();drop_create("DROP $Kg ".routine_id($da,$vf),create_routine($Kg,$J),"DROP $Kg ".routine_id($J["name"],$J),create_routine($Kg,array("name"=>$Nh)+$J),"DROP $Kg ".routine_id($Nh,$J),substr(ME,0,-1),'Procedura byla odstranÄ›na.','Procedura byla zmÄ›nÄ›na.','Procedura byla vytvoÅ™ena.',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmÄ›nit funkci':'ZmÄ›nit proceduru').": ".h($da):(isset($_GET["function"])?'VytvoÅ™it funkci':'VytvoÅ™it proceduru')),$n);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Kg);$J["name"]=$da;}$pb=get_vals("SHOW CHARACTER SET");sort($pb);$Lg=routine_languages();echo'
<form action="" method="post" id="form">
<p>NÃ¡zev: <input name="name" value="',h($J["name"]),'" maxlength="64" autocapitalize="off">
...
<input type="submit" value="UloÅ¾it">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',$da));}echo'<input type="hidden" name="token" value="',$ci,'">
</form>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$ni=trigger_options();$J=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$ni["Timing"])&&in_array($_POST["Event"],$ni["Event"])&&in_array($_POST["Type"],$ni["Type"])){$gf=" ON ".table($a);$ec="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$gf:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($ec,$A,'Trigger byl odstranÄ›n.');else{if($C!="")queries($ec);queries_redirect($A,($C!=""?'Trigger byl zmÄ›nÄ›n.':'Trigger byl vytvoÅ™en.'),queries(create_trigger($gf,$_POST)));if($C!="")queries(create_trigger($gf,$J+array("Type"=>reset($ni["Type"]))));}}$J=$_POST;}page_header(($C!=""?'ZmÄ›nit trigger'.": ".h($C):'VytvoÅ™it trigger'),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>ÄŒas<td>',html_select("Timing",$ni["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>UdÃ¡lost<td>',html_select("Event",$ni["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$ni["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>Typ<td>',html_select("Type",$ni["Type"],$J["Type"]),'</table>
...
<input type="submit" value="UloÅ¾it">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit">',confirm(sprintf('Odstranit %s?',"$ha@$_GET[host]"));}echo'<input type="hidden" name="token" value="',$ci,'">
</form>
